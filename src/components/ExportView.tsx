...
export default function ExportView({ sections, creditMap }: { sections: Section[]; creditMap: Record<string, number | null> }) {
  ...
  async function handleShare() {
    ...
    if (navigator.share) {
      try {
        await navigator.share({ title: "My ITU Schedule", url });
      } catch {
  ...
  return (
    ...
    <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 shadow-lg z-10">
      <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
        ...
        <button
          onClick={handleShare}
          className="flex items-center gap-2 px-4 py-2 text-sm rounded-lg bg-[#1a2b4e] text-white hover:bg-[#253d6e] transition-colors"
        >
          {shared ? (
            "Link copied!"
          ) : (
